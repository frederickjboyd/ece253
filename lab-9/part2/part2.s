/* 
 * Program that displays sweeping LEDs which move left and right
 * Sweeping stops if KEY3 is pressed
 * Timer and KEYS are handled via polled I/O
 */
...
ROT_LEFT:
				CMP		R6, #0x00000100	// Check if left most LED is lit up
				MOVEQ	R3, #1			// If left most LED is lit, change direction
            	LSL		R6, #1			// Rotate left
            	B		DELAY_1
ROT_RIGHT:
				CMP		R6, #0x00000002	// Check if right most LED is lit up
